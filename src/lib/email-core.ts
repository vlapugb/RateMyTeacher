...
export type EmailProvider = "smtp" | "gmail" | "sendgrid" | "brevo" | "mailgun";
...
export function requireEmailEnv(key: string) {
...
async function assertEmailApiResponse(response: Response, provider: string) {
  if (!response.ok) {
...
export async function getGmailAccessToken() {
  const response = await fetchWithEmailTimeout("https://oauth2.googleapis.com/token", {
    ...
    body: new URLSearchParams({
      ...
      refresh_token: requireEmailEnv("GMAIL_REFRESH_TOKEN"),
...
export async function sendViaSmtp(payload: EmailPayload) {
...
export async function sendViaSendGrid(payload: EmailPayload) {
  const apiKey = requireEmailEnv("SENDGRID_API_KEY");
...
export async function sendViaBrevo(payload: EmailPayload) {
...
export async function sendViaMailgun(payload: EmailPayload) {
  const apiKey = requireEmailEnv("MAILGUN_API_KEY");
  const domain = requireEmailEnv("MAILGUN_DOMAIN");
...
export async function sendViaGmail(payload: EmailPayload) {
